...
-- |
-- Module      :  Network.HTTP
-- Copyright   :  (c) Warrick Gray 2002, Bjorn Bringert 2003-2004
-- License     :  BSD
-- 
...
instance Stream Socket where
    ...
    writeBlock sk str = (liftM Right $ fn str) `Exception.catch` (handleSocketError sk)
        where
            fn [] = return ()
            fn x  = send sk str >>= \i -> fn (drop i str)
        ...
        where
            fn sk addr s
                ...
                | otherwise =
                    ...
                    if se == 0
                        then sendTo sk str addr >>= \i -> fn sk addr (drop i str)
